  ...
  const handleCreateCampaign = async () => {
    ...
    try {
      const res = await fetch(`${API}/crm/campaigns`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: campForm.name, list_id: campForm.list_id || null,
    ...
    <View style={s.contentInner}>
      <View style={s.flexBetween}>
        <View>
          ...
          <Text style={s.pageSubtitle}>Gerencie e dispare mensagens em lote.</Text>
        </View>
      ...
      {/* Mesma lógica de formulário da versão anterior, adaptada para o card... */}
      {showNewCamp && (
        <View style={s.card}>
          <Text style={s.cardTitle}>Nova Campanha</Text>
          <TextInput style={s.input} placeholder="Nome da campanha *" value={campForm.name} onChangeText={v => setCampForm(f => ({...f, name:v}))} />
          
          <Text style={s.label}>Lista de Audiência:</Text>
          <View style={{ flexDirection:'row', gap:6, flexWrap:'wrap', marginBottom:12 }}>
            {lists.map(l => (
              <TouchableOpacity key={l.id} style={[s.chip, campForm.list_id === String(l.id) && s.chipActive]} onPress={() => setCampForm(f => ({...f, list_id: String(l.id)}))}>
                <Text style={[s.chipTxt, campForm.list_id === String(l.id) && s.chipTxtActive]}>{l.name} ({l.contact_count})</Text>
              </TouchableOpacity>
            ))}
          </View>

          <Text style={s.label}>Template Meta:</Text>
          <View style={{ flexDirection:'row', gap:6, flexWrap:'wrap', marginBottom:12 }}>
            {metaTemplates.map(t => (
          ...
          <View style={{ flexDirection:'row', gap:8, marginTop:10 }}>
            <TouchableOpacity style={s.btnPrimary} onPress={handleCreateCampaign} disabled={savingCamp}>
              <Text style={s.btnPrimaryTxt}>{savingCamp ? 'Salvando...' : 'Salvar Rascunho'}</Text>
            </TouchableOpacity>
            ...
            <View key={camp.id} style={s.card}>
              <View style={s.flexBetween}>
                <View>
                  ...
                  <Text style={{ fontSize:12, color:'#64748B', marginBottom:8 }}>
                    {list?.name || 'Sem lista'} • {camp.template_name} • {fmtDate(camp.created_at)}
                  </Text>
                  {camp.status === 'done' && (
                     <Text style={{ fontSize:12, color:'#16A34A', fontWeight:'700' }}>✓ {camp.sent_count} enviados, {camp.error_count} erros.</Text>
                  )}
                </View>
                <View style={{ gap:8 }}>
                  {camp.status === 'draft' && (
                    <TouchableOpacity style={[s.btnLaunch, !canLaunch && { backgroundColor:'#94A3B8' }]} onPress={() => canLaunch && handleLaunchCampaign(camp.id)}>
                      <Zap size={14} color="#fff" /><Text style={{ color:'#fff', fontWeight:'700', fontSize:12 }}>Lançar via Meta</Text>
                    </TouchableOpacity>
                  )}
                  <TouchableOpacity style={s.btnSecondary} onPress={() => openCampaignLogs(camp)}>
                    <BarChart2 size={13} color="#475569" /><Text style={s.btnSecondaryTxt}>Logs</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => handleDeleteCampaign(camp.id)}>
                    <Text style={{ color:'#DC2626', fontSize:12, textAlign:'right' }}>Excluir</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              {selectedCamp?.id === camp.id && (
                <View style={{ marginTop:16, borderTopWidth:1, borderColor:'#E2E8F0', paddingTop:12 }}>
                  <Text style={{ fontWeight:'700', marginBottom:8 }}>Logs de Envio:</Text>
                  {campLogs.map(log => (
                    <View key={log.id} style={{ flexDirection:'row', borderBottomWidth:1, borderColor:'#F1F5F9', paddingVertical:4 }}>
                      <Text style={{ flex:1, fontSize:12 }}>{log.name}</Text>
                      <Text style={{ flex:1, fontSize:12 }}>{log.phone}</Text>
                      <Text style={{ flex:1, fontSize:12, color: log.status==='sent' ? '#16A34A':'#DC2626' }}>{log.status}</Text>
                      <Text style={{ flex:1, fontSize:11, color:'#94A3B8' }}>{log.wamid?.slice(0,15) || log.error_msg}</Text>
                    </View>
                  ))}
                </View>
              )}
